...
class StaffRegisterStudentForm extends FormRequest
{
    /**
      ...
      $user = Auth::user()->owner;
       ...
       if ($this->avatar && $this->hasFile('avatar_image')) {
           ...
           if ($this->avatar) {
            ...
            $filename= time() . ".png";
            $avatar_s ='https://s3.ap-south-1.amazonaws.com/dbmszar/student/avatar'.'/'.$user->id.'/'.$this->reg_no.'/'. $filename;
            ...
            if(!empty($img_data)){
                ...
                Storage::disk('s3')->put("student/avatar/$user->id/$this->reg_no/$filename", $img_data);
            }
            ...
            Storage::disk('s3')->put("student/avatar/$user->id/$this->reg_no/$filename", $image->__toString());
        ...
        //address requesting
         ...
         $number     = $this->parent_no;
         $school     = $user->schoolprofile->school_name;


        $user->students()->create($data);

        event(new StudentRegister($number,$regno,$password,$school));

         flash()->success('Successfully Student Created and your Reg No. is '. $regno . ' and password: ' . $password);
