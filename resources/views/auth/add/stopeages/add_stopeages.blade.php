...
@section('content')
 ...
 <div class="row">
 	...
 	<div class="col-md-8">
 	    <div class="panel panel-default">
            <div class="panel-heading">
                <button class="btn btn-primary btn-block">Stoppages</button>
            </div>
			            ...
			            @foreach($stopeages as $stopeage)
			             <?php $i++ ?>
			              <tr>
			                  <td>{{ $i }}</td>
			                  <td>{{ $stopeage->name }}</td>
			                  <td>@include('auth.add.stopeages.stopeages_busdetail_modal')</td>
			                  <td>
			                      @if($stopeage->remarks)
				                   {{ $stopeage->remarks }}
				                   @else
			                  ...
			                  <td>@include('auth.add.stopeages.stopeages_update_modal')</td>
			              </tr>
			            @endforeach
			            </tbody>
		            </table>
    ...
    <div class="col-md-4">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <button class="btn btn-primary btn-block">Stoppages Create Form</button>
            </div>
            <div class="panel-body">

		  	    <form method="POST" action="{{ route('stopages_auth.store') }}" data-parsley-validate ="">
		        {{ csrf_field() }}
